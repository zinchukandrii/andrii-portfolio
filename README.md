# Andrii Zinchuk — Engineering Systems Portfolio

Static portfolio for two public, verified software projects, each with a recruiter-facing case study:

- [Evidence-First Agent Knowledge & Control Plane](projects/control-plane.html)
- [Resilient Runtime Failover Configuration](projects/runtime-failover.html)

## Local preview

```bash
python3 -m http.server 8080
```

Open `http://127.0.0.1:8080`.

## Public-data boundary

This site contains only public project links and profile details already cleared in the portfolio source. It does not include a CV PDF, employer/client data, job-search automation, credentials or production logs.

## Deploy

The repository is static and deploys to GitHub Pages from `main` / root.
